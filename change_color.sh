...
CANTA_STYLE_COMPACT=$(echo ${CANTA_STYLE_COMPACT-True} | tr '[:upper:]' '[:lower:]')
CANTA_MENUBAR_STYLE=$(echo ${CANTA_MENUBAR_STYLE-same} | tr '[:upper:]' '[:lower:]')
...
rm ./src/gtk/*.css
if [[ ${CANTA_MENUBAR_STYLE}  == "contrast" ]] ; then
	rm ./src/gtk/gtk-light*.scss
else
	rm ./src/gtk/gtk{,-compact}.scss || true
fi
if [[ ${GTK3_GENERATE_DARK} != "true" ]] ; then
	grep -v "\-dark" ./src/gtk/assets.txt > ./new_assets.txt
	mv ./new_assets.txt ./src/gtk/assets.txt
	rm ./src/gtk/gtk-dark-compact.scss
	rm ./src/gtk/gtk-dark.scss
fi
...
if [[ ${CANTA_STYLE_COMPACT}  == "true" ]] ; then
